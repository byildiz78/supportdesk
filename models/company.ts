...
export class CompanyModel {
  ...
  /**
   * Ana şirket ID'sine göre şirketleri listeler
   */
  static async findByParentCompanyId(parentCompanyId: string): Promise<Company[]> {
    const result = await db.query(
      'SELECT * FROM companies WHERE parent_company_id = $1 AND is_deleted = false ORDER BY name',
      [parentCompanyId]
    );
    return result.rows;
  }
  ...
  static async findByName(name: string): Promise<Company | null> {
    const result = await db.query(
      'SELECT * FROM companies WHERE name = $1 AND is_deleted = false',
      [name]
    );
    return result.rows[0] || null;
  }
  ...
  /**
   * Toplam şirket sayısını döndürür
   */
  static async count(options: {
    ...
    let query = 'SELECT COUNT(*) FROM companies WHERE is_deleted = false';
    const params: any[] = [];
    let paramIndex = 1;
  ...
  static async create(companyData: Omit<Company, 'id' | 'created_at' | 'updated_at'>): Promise<Company> {
    ...
    const result = await db.query(
      `INSERT INTO companies (
        id, parent_company_id, name, tax_id, tax_office, address, city, state, postal_code,
      ...
      [
        id, companyData.parent_company_id, companyData.name, companyData.tax_id, companyData.tax_office,
  ...
  static async update(id: string, companyData: Partial<Company>): Promise<Company | null> {
    ...
    try {
      ...
      // Önce mevcut şirketi kontrol et
      ...
      if (companyData.parent_company_id !== undefined) {
        updates.push(`parent_company_id = $${paramIndex}`);
        values.push(companyData.parent_company_id);
        paramIndex++;
      }
      ...
      // Eğer güncellenecek alan yoksa, mevcut şirketi döndür
      if (updates.length === 0) {
        await client.query('ROLLBACK');
