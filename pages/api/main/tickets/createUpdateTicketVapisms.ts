...
export default async function handler(
  ...
  try {
    ...
    return await db.executeTransaction(req, async (client) => {
      ...
      // Eğer ticket başarıyla oluşturulduysa ve bir telefon numarası varsa SMS gönder
      if (ticketId && safeTicketData.customer_phone) {
        console.log('Starting SMS notification process for new ticket:', {
          ticketId,
          ticketNo,
          customerPhone: safeTicketData.customer_phone
        });
        
        try {
          // Telefon numarasını formatla (başında 90 olacak şekilde)
          let phoneNumber = safeTicketData.customer_phone;
          if (phoneNumber.startsWith('+')) {
            phoneNumber = phoneNumber.substring(1);
          }
          if (!phoneNumber.startsWith('90') && phoneNumber.startsWith('0')) {
          ...
          // Aşama 1: Müşteri numarasını servise gönder
          const updateContactResponse = await fetch('https://crm.robotpos.com/rest/1/q5w7kffwsbyyct5i/crm.contact.update', {
            method: 'POST',
            headers: {
          ...
          console.log('Contact update response:', updateContactResult);
          
          // 3 saniye bekle
          await new Promise(resolve => setTimeout(resolve, 3000));
          
          // SMS içeriğini hazırla
          let smsTitle = `robotpos destek talebiniz #${ticketNo}# no ile olusturulmustur. Kisa sure icerisinde uzman bir arkadasimiz sizinle iletisime gececektir.`;
          
          // Aşama 2: SMS içeriğini gönder
          const sendSmsResponse = await fetch('https://crm.robotpos.com/rest/1/q5w7kffwsbyyct5i/crm.item.add', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({
              ...
              "fields": {
                ...
                "contactId": 2,
                "companyId": 4,
                "ufCrm18_1729171116072": "sms"
              }
            })
          });
          
          const sendSmsResult = await sendSmsResponse.json();
          console.log('SMS sending response:', sendSmsResult);
          
          // Aşama 3: SMS bildirimini dahili yorum olarak kaydet
          if (sendSmsResult && sendSmsResult.result && sendSmsResult.result.item) {
            console.log('SMS sent successfully, saving as internal comment');
            // Sistem kullanıcısı ID'si
            const systemUserId = "efa579e5-6d64-43d0-b12a-a078ab357e90";
            
            // SMS bildirimini dahili yorum olarak ekle
            const insertSmsCommentQuery = `
              INSERT INTO ticket_comments (
                ticket_id,
                content,
                is_internal,
            ...
            console.log('Saving SMS notification as internal comment for ticket:', ticketId);
            await client.query(insertSmsCommentQuery, [
              ticketId,
              `SMS bildirimi gönderildi: "${smsTitle}" - Telefon: ${phoneNumber}`,
              systemUserId
            ]);
            
            console.log('SMS notification comment saved successfully');
          } else {
            console.error('SMS sending failed or incomplete response:', sendSmsResult);
            
            // SMS gönderimi başarısız olsa bile dahili yorum ekle
            try {
              const systemUserId = "efa579e5-6d64-43d0-b12a-a078ab357e90";
              const insertErrorCommentQuery = `
                INSERT INTO ticket_comments (
                  ticket_id,
                  content,
                  is_internal,
              ...
              console.log('Saving SMS error as internal comment for ticket:', ticketId);
              await client.query(insertErrorCommentQuery, [
                ticketId,
                `SMS bildirimi gönderme denemesi yapıldı fakat bir sorun oluştu. Telefon: ${phoneNumber}`,
                systemUserId
              ]);
              
              console.log('SMS error comment saved successfully');
            } catch (commentError) {
              console.error('Failed to save SMS error comment:', commentError);
            }
          }
        } catch (smsError) {
          console.error('Error in SMS notification process:', smsError);
          // SMS gönderme hatası olsa bile işleme devam et
        }
      } else {
        console.log('Skipping SMS notification - conditions not met:', {
          hasTicketId: !!ticketId,
          hasCustomerPhone: !!safeTicketData.customer_phone
        });
      }
