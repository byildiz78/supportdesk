...
export default async function handler(
  ...
  t.description,
  CASE 
    WHEN t.status = 'deleted' OR t.is_deleted = true THEN 'deleted'
    ELSE t.status 
  END as status,
  t.priority,
  t.source,
...
LEFT JOIN users creator ON t.created_by::uuid = creator.id
LEFT JOIN users updater ON t.updated_by::uuid = updater.id
